            ...
            <div class="card card-primary card-outline">
                <form role="form" action="{{ route('updateBayiBalita', $bayiBalita->id_bb) }}" method="POST">
                    ...
                    <div class="card-body">
                      ...
                      <div class="form-group">
                        ...
                        <div class="d-flex">
                            <div class="custom-control custom-radio mr-3">
                                <input class="custom-control-input" type="radio" id="laki-laki" name="jenis_kelamin" value="Laki-laki" {{ $bayiBalita->jenis_kelamin == 'Laki-laki' ? 'checked' : ''}}>
                                <label for="laki-laki" class="custom-control-label">Laki - Laki</label>
                            </div>
                            <div class="custom-control custom-radio">
                                <input class="custom-control-input" type="radio" id="perempuan" name="jenis_kelamin" value="Perempuan" {{ $bayiBalita->jenis_kelamin == 'Perempuan' ? 'checked' : ''}}>
                      ...
                      <div class="form-group">
                        <label for="ibu"><strong>Nama Kader</strong></label>
                        <select class="form-control select2bs4" name="ibu" id="ibu" style="width: 100%;" required><br>
                        @foreach ($ibu as $item)
                            <option value="{{ $item->id }}" {{ $bayiBalita->id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                        @endforeach
                        </select>
                      </div>
                      <div class="form-group">
